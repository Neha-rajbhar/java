...
public class RemoveFromCart extends HttpServlet {
	private static final long serialVersionUID = 1L;
   	...
   	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * response.getWriter()) { String bookId = request.getParameter("id"); if
		 * (bookId != null) { ArrayList<Cart> cart_list = (ArrayList<Cart>)
		 * request.getSession().getAttribute("cart-list"); if (cart_list != null) { for
		 * (Cart c : cart_list) { if (c.getId() == Integer.parseInt(bookId)) {
		 * cart_list.remove(cart_list.indexOf(c)); break; } } }
		 * response.sendRedirect("usercartdisplay.jsp");
		 * 
